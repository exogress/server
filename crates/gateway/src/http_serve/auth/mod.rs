...
pub async fn async_http_client(
    ...
    let headers = response
        ...
        .map(|(name, value)| {
            (
                http_1::header::HeaderName::from_bytes(name.as_str().as_ref())
                    .expect("failed to convert HeaderName from http 0.2 to 0.1"),
                http_1::header::HeaderValue::from_bytes(value.as_bytes())
                    .expect("failed to convert HeaderValue from http 0.2 to 0.1"),
            )
        })
        .collect::<http_1::HeaderMap>();
    let chunks = response.bytes().await.map_err(Error::Reqwest)?;
    Ok(oauth2::HttpResponse {
        status_code: http_1::StatusCode::from_u16(status_code.as_u16())
